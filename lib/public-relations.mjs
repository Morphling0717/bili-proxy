...
async function fetchRelationTotal({ mid, kind, timeoutMs, userAgent }) {
  ...
  try {
    ...
    if (!diagnostic.ok) {
      return {
        ...
        diagnostic: {
          ...diagnostic,
          message: json?.message || json?.msg || `HTTP ${response.status}`,
        },
      };
    }

    const candidate =
      kind === 'following' ? json?.data?.following : json?.data?.follower;
    const total =
      candidate !== null &&
      candidate !== undefined &&
      candidate !== '' &&
...
export async function fetchPublicRelations({
  ...
  let emptyWithoutEvidence = false;
  ...
  for (let index = 0; index < pagesAllowed; index += 1) {
    ...
    if (
      ...
      const statistic = await fetchRelationTotal({
        mid,
        kind,
        timeoutMs: clamp(timeoutMs, 3_000, 30_000),
        userAgent,
      });
      relationStat = statistic.diagnostic;
      if (statistic.total !== null) resolvedTotal = statistic.total;
    }
  ...
  else if (emptyWithoutEvidence) completeness = 'empty_or_private';
  ...
  return {
    data: {
      ...
      privacy_restricted: emptyWithoutEvidence ? null : privacyRestricted,
      capped,
      completeness,
    },
    diagnostics: {
