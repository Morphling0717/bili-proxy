  ...
  try {
    ...
    if (!response.ok || !json || json.code !== 0) {
      throw new PublicRelationError(
        ...
        {
          ...diagnostic,
          body_hint: json ? undefined : text.slice(0, 160),
        },
      );
    }
...
export async function fetchPublicRelations({
  ...
  const items = [];
  let publicListUnavailable = false;
  let indeterminateEmpty = false;
  ...
  for (let index = 0; index < pagesAllowed; index += 1) {
    ...
    const inferredPrivate =
      emptyFirstPage && resolvedTotal !== null && resolvedTotal > 0;
    if (result.privacyRestricted || (inferredPrivate && kind === 'following')) {
      privacyRestricted = true;
      upstreamHasMore = false;
      break;
    }
    if (inferredPrivate && kind === 'followers') {
      publicListUnavailable = true;
      upstreamHasMore = false;
      break;
    }
    if (emptyFirstPage && resolvedTotal === null) {
      publicListUnavailable = true;
      indeterminateEmpty = true;
      upstreamHasMore = false;
      break;
    }
  ...
  const pageBudgetExhausted =
    complete &&
    diagnostics.requests.length >= pagesAllowed &&
    upstreamHasMore &&
    !capped;
  const upstreamLimited =
    !privacyRestricted &&
    !publicListUnavailable &&
    !capped &&
    resolvedTotal !== null &&
  ...
  else if (publicListUnavailable) {
    completeness = indeterminateEmpty
      ? 'indeterminate_empty_list'
      : 'public_list_unavailable';
  ...
  else if (!hasMore && resolvedTotal !== null && endOffset >= resolvedTotal) {
  ...
  return {
    data: {
      ...
      total: resolvedTotal,
      accessible_total:
        privacyRestricted || publicListUnavailable
          ? 0
          : upstreamLimited
            ? endOffset
            : resolvedTotal !== null
      ...
      privacy_restricted: indeterminateEmpty ? null : privacyRestricted,
      public_list_unavailable: publicListUnavailable,
      capped,
      completeness,
    },
    diagnostics,
